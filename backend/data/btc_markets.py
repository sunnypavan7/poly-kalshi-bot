...
# Strict regex: only match real BTC 5-min window slugs
...
async def fetch_btc_market_by_slug(slug: str) -> Optional[BtcMarket]:
    ...
    if not is_valid_btc_slug(slug):
        logger.debug("Rejected invalid BTC slug: %s", slug)
        return None
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            logger.debug("Failed to fetch BTC market %s: %s", slug, e)
            return None
...
async def fetch_active_btc_markets() -> List[BtcMarket]:
    ...
    try:
        ...
        logger.debug("BTC series search fallback failed: %s", e)
    ...
    logger.info("Fetched %d active BTC 5-min markets", len(markets))
    return markets
...
async def fetch_btc_market_for_settlement(slug: str) -> Optional[BtcMarket]:
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            logger.warning("Failed to fetch BTC market for settlement %s: %s", slug, e)
            return None
