...
async def scan_for_weather_signals() -> List[WeatherTradingSignal]:
    ...
    try:
        ...
        logger.info("Polymarket: %d weather markets", len(poly_markets))
    except Exception as e:
        logger.error("Failed to fetch Polymarket weather markets: %s", e)
    ...
    if settings.KALSHI_ENABLED:
        try:
            ...
            logger.error("Failed to fetch Kalshi weather markets: %s", e)

    logger.info("Found %d total weather temperature markets", len(markets))
    ...
    for market in markets:
        try:
            ...
            logger.debug("Weather signal generation failed for %s: %s", market.title, e)

    signals.sort(key=lambda s: abs(s.edge), reverse=True)

    actionable = [s for s in signals if s.passes_threshold]
    logger.info("WEATHER SCAN COMPLETE: %d signals, %d actionable", len(signals), len(actionable))

    for signal in actionable[:5]:
        logger.info("  %s: %s %s %sF | Edge: %+.1f%%",
                     signal.market.city_name, signal.market.metric, signal.market.direction,
                     signal.market.threshold_f, signal.edge * 100)
...
def _persist_weather_signals(signals: list):
    ...
    try:
        ...
        logger.warning("Failed to persist weather signals: %s", e)
        db.rollback()
    finally:
        db.close()
