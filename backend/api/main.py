"""FastAPI backend for unified BTC 5-min + Weather trading bot dashboard."""
...
app = FastAPI(
    title="Poly-Kalshi Trading Bot",
    description="Polymarket + Kalshi weather & BTC 5-min trading bot",
    version="3.0.0"
)
...
async def startup():
    ...
    logger.info("=" * 60)
    logger.info("POLY-KALSHI TRADING BOT v3.0")
    logger.info("=" * 60)
    ...
    try:
        ...
        if not state:
            ...
            db.add(state)
            db.commit()
            logger.info("Created new bot state with $%.2f bankroll", settings.INITIAL_BANKROLL)
        else:
            state.is_running = True
            db.commit()
            logger.info("Loaded bot state: Bankroll $%.2f, P&L $%+.2f, %d trades",
                        state.bankroll, state.total_pnl, state.total_trades)
    finally:
        db.close()

    logger.info("Configuration:")
    logger.info("  - Simulation mode: %s", settings.SIMULATION_MODE)
    logger.info("  - Min edge (BTC): %.0f%%", settings.MIN_EDGE_THRESHOLD * 100)
    logger.info("  - Min edge (Weather): %.0f%%", settings.WEATHER_MIN_EDGE_THRESHOLD * 100)
    logger.info("  - Kelly fraction: %.0f%%", settings.KELLY_FRACTION * 100)
    logger.info("  - Scan interval: %ds", settings.SCAN_INTERVAL_SECONDS)

    from backend.core.scheduler import start_scheduler, log_event
    start_scheduler()
    log_event("success", "Trading bot initialized")

    logger.info("Bot is now running!")
    logger.info("  - BTC scan: every %ds (edge >= %.0f%%)",
                settings.SCAN_INTERVAL_SECONDS, settings.MIN_EDGE_THRESHOLD * 100)
    logger.info("  - Settlement check: every %ds", settings.SETTLEMENT_INTERVAL_SECONDS)
    if settings.WEATHER_ENABLED:
        logger.info("  - Weather scan: every %ds (edge >= %.0f%%)",
                    settings.WEATHER_SCAN_INTERVAL_SECONDS, settings.WEATHER_MIN_EDGE_THRESHOLD * 100)
        logger.info("  - Weather cities: %s", settings.WEATHER_CITIES)
    else:
...
async def root():
    return {
        "status": "ok",
        "message": "Poly-Kalshi Trading Bot API v3.0",
...
@app.get("/api/btc/microstructure", response_model=Optional[MicrostructureResponse])
async def get_microstructure():
    """Get current BTC microstructure indicators."""
    try:
...
async def get_dashboard(db: Session = Depends(get_db)):
    ...
    # BTC price from microstructure, fallback to CoinGecko
    btc_price_data = None
    micro_data = None
    try:
        micro = await compute_btc_microstructure()
